                ...
                children: [
                  GestureDetector(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text('Search').text.fontFamily(medium).color(greyDark).size(16).make(),
                        Icon(Icons.search, color: greyDark),
                      ],
                    ).box.width(context.screenWidth - 140).padding(EdgeInsets.symmetric(horizontal: 16, vertical: 14)).border(color: greyLine).roundedLg.make(),
                  ),
                  15.widthBox,
                  IconButton(
                    icon: Icon(Icons.filter_list_rounded, color: greyDark, size: 30),
                    onPressed: () {
                      showModalRightSheet(
                        context: context,
                        builder: (BuildContext context) {
            ...
            Expanded(
              child: FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (currentUser != null) {
                    productsToShow = getRandomizedList(products.where((product) => !isInWishlist(product, currentUser!.uid)).toList());
                  } else {
                  ...
                  return CardSwiper(
                        ...
                        child: Column(
                          children: [
                            Container(
                              child: Stack(
                                children: [
                                  ...
                                  ).box.white.rounded.outerShadow.padding(EdgeInsets.all(12)).make(),
                                ],
                              ),
                            ),
                            SizedBox(height: 5),
                            Row(
                              ...
                              children: [
                                IconButton(
                                  icon: Image.asset(icDislikeButton, width: 60).box.roundedFull.outerShadow.make(),
                                  onPressed: () => controllercard.swipe(CardSwiperDirection.left),
                                ),
                                SizedBox(width: 30),
                                IconButton(
                                  icon: Image.asset(icViewMoreButton, width: 60).box.roundedFull.outerShadow.make(),
                                  onPressed: () => navigateToItemDetails(),
                                ),
                                SizedBox(width: 30),
                                IconButton(
                                  icon: Image.asset(icLikeButton, width: 60).box.roundedFull.outerShadow.make(),
                                  onPressed: () {
                    ...
                    onSwipe: (previousIndex, currentIndex, direction) {
                      ...
                      if (currentIndex != null) {
                        Map<String, dynamic>? currentProduct = productsToShow[currentIndex];
